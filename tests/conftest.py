...
"""Test configuration and utilities for v8x tests.

Test isolation strategy:
1. Create a temporary directory and globally patch V8X_LOCAL_USER_BASE_DIR
   to point to it for all tests.
2. Write a minimal default profile config and directory structure.
3. Never touch the real ~/.v8x.
...
_test_home = Path(tempfile.mkdtemp(prefix="v8x_test_"))
...
def _global_patch_base_dir():
    """Globally patch V8X_LOCAL_USER_BASE_DIR for all tests."""
    with patch("v8x.constants.V8X_LOCAL_USER_BASE_DIR", _test_home):
                    ...
                    with patch(
                        ...
                        with patch(
                            ...
                            with patch(
                                ...
                                with patch(
                                    "v8x.config.V8X_ACTIVE_PROFILE",
                                    _test_home / "active_profile",
                                ):
                                    with patch(
                                        "v8x.constants.V8X_DEPLOYMENTS_CACHE_PATH",
...
def override_cache_dir(tmp_path, mocker):
    ...
    mocker.patch("v8x.cache.USER_CACHE_DIR", tmp_path)
    mocker.patch("v8x.cache.USER_TOKEN_DIR", tmp_path / "tokens")
...
def mock_json_bypass():
    ...
    """
    with patch("v8x.render.RenderStepOutput.json_bypass") as mock:
        yield mock
...
def mock_graphql_client() -> Generator[MockGraphQLClient, None, None]:
    ...
    with patch("v8x.gql_client.create_async_graphql_client", return_value=mock_client):
        # Also patch any import paths where the factory might be used, but tolerate missing attributes
        optional_patch_targets = [
            "v8x.commands.cluster.create.create_async_graphql_client",
            "v8x.commands.cluster.list.create_async_graphql_client",
            "v8x.commands.cluster.delete.create_async_graphql_client",
            "v8x.commands.cluster.utils.create_async_graphql_client",
        ]
