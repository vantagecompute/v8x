...
- **v8x**: Streamlined VM and cluster management
...
```bash
v8x app deploy slurm-multipass
```
...
v8x app status slurm-multipass
...
v8x app config slurm-multipass \
...
```bash
# Start VM
v8x app start slurm-multipass
# OR
multipass start slurm-node

# Stop VM
v8x app stop slurm-multipass
# OR
multipass stop slurm-node

# Restart VM
v8x app restart slurm-multipass
# OR
multipass restart slurm-node

# Delete VM
v8x app delete slurm-multipass
# OR
...
v8x app deploy slurm-multipass --cpus=8 --memory=16G
```
...
```bash
# Deploy multiple VMs
v8x app deploy slurm-multipass --name=head-node
v8x app deploy slurm-multipass --name=compute-node-1 --compute-only
v8x app deploy slurm-multipass --name=compute-node-2 --compute-only

# Configure cluster networking
v8x app config slurm-multipass --cluster-nodes=head-node,compute-node-1,compute-node-2
```
...
v8x app deploy slurm-multipass --cloud-init=./custom-init.yaml
```
