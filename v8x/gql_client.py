...
from vantage_sdk.exceptions import VantageCliError

from .auth import extract_persona, is_token_expired, refresh_access_token_standalone
...
class VantageGraphQLClient:
    ...
    def _validate_auth(self) -> None:
        """Validate authentication, proactively refreshing if near expiry."""
        if not self.persona:
        ...
        if self._is_token_expired():
            if self.settings and self.persona.token_set.refresh_token:
                success = refresh_access_token_standalone(self.persona.token_set, self.settings)
                if success:
                    save_tokens_to_cache(self.profile, self.persona.token_set)
                    self._refresh_transport_headers()
                    logger.debug("Proactively refreshed token and re-initialized transport")
                    return
            raise AuthenticationError("Access token has expired")

    def _is_token_expired(self) -> bool:
        """Check if current token is expired or near expiry."""
        if not self.persona or not self.persona.token_set.access_token:
            return True
        return is_token_expired(self.persona.token_set.access_token)
...
class VantageGQLClient:
    ...
    def create(self) -> VantageGraphQLClient:
        ...
        try:
            ...
            logger.debug(f"Failed to create async GraphQL client: {e}")
            raise
