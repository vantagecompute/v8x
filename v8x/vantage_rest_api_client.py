...
from .auth import is_token_expired, refresh_access_token_standalone
...
class VantageRestApiClient:
    ...
    async def _refresh_token_if_needed(self) -> bool:
        """Refresh access token if near expiry and recreate HTTP client."""
        ...
        try:
            ...
            if refresh_success:
                ...
                await self._reinit_client()
                logger.debug("Successfully refreshed access token and re-initialized HTTP client")
                return True
            return False
    ...
    async def _reinit_client(self) -> None:
        """Close and recreate the HTTP client with the refreshed token."""
    ...
    async def _ensure_fresh_token(self) -> None:
        ...
        if (
            self.persona
            and self.persona.token_set.access_token
            and is_token_expired(self.persona.token_set.access_token)
        ):
