...
"""Token caching functionality for the v8x."""
...
from v8x.constants import USER_TOKEN_CACHE_DIR
...
def init_cache(cache_dir: Path) -> None:
    ...
    try:
        ...
        info_file.write_text("This directory is used by v8x for its cache.")
...
def _get_token_paths(profile: str) -> tuple[Path, Path, Path]:
    ...
    id_token_path: Path = token_dir / "id.token"
    return (access_token_path, refresh_token_path, id_token_path)


def load_tokens_from_cache(profile: str) -> TokenSet:
    """Load access token and refresh token from the cache."""
    (access_token_path, refresh_token_path, id_token_path) = _get_token_paths(profile)

    Abort.require_condition(
        access_token_path.exists(),
        "Please login with your auth token first using the `v8x login` command",
    ...
    if id_token_path.exists():
        logger.debug("Retrieving ID token from cache")
        token_set.id_token = id_token_path.read_text()
...
def save_tokens_to_cache(profile: str, token_set: TokenSet):
    ...
    if token_set.id_token is not None:
        logger.debug(f"Caching ID token at {id_token_path}")
        id_token_path.write_text(token_set.id_token)
        id_token_path.chmod(0o600)
...
def clear_token_cache(profile: str):
    ...
    logger.debug(f"Removing ID token at {id_token_path}")
    if id_token_path.exists():
        id_token_path.unlink()
