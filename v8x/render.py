...
class TableViewerApp(App):
    ...
    def _format_cell_value(self, key: str, value: Any) -> str:
        ...
        if value is None:
            ...
            # For complex nested data, show a summary or expand for specific keys
            if isinstance(value, list):
                return f"[{len(value)} items]" if value else "[]"
            else:
                # Expand settings/attributes dict to show key=value pairs
                if key.lower() in ("settings", "attributes") and value:
                    lines = []
                    for k, v in sorted(value.items()):
                        # Truncate long values (like certs/keys)
...
class RenderStepOutput:
    ...
    def show_quick_start(self):
        ...
        message = snick.dedent(
            """
            • To view cluster details, use the command: v8x clusters get

            • To create a new cluster, use the command: v8x clusters create --help

            • For more information on any command run it with the --help option.

            • To check all the available commands, refer to: v8x --help
            """
        ).strip()

        panel = Panel(
            message,
            title="[bold magenta]Quick Start Guide for v8x[/bold magenta]",
            border_style="blue",
            expand=False,
        )
...
class UniversalOutputFormatter:
    ...
    def output(
        ...
        Args:
            ...
            preferred_fields: Optional field order to prioritize in tabular output
        """
        if self.json_output:
            self._output_json(data)
        else:
            self._output_table(data, title, empty_message, preferred_fields)
    ...
    def _output_table(
        ...
        if isinstance(data, dict):
            if "items" in data and isinstance(data["items"], list):
                ...
                self._render_list_as_table(data["items"], title, preferred_fields)
            ...
            if len(data) == 0:
                ...
                self._render_list_as_table(data, title, preferred_fields)
        else:
            # Simple value
    ...
    def _render_list_as_table(
        ...
        self._render_textual_style_table(items, title, preferred_fields)

    def _render_textual_style_table(
        self,
        items: List[Dict[str, Any]],
        title: str,
        preferred_fields: Optional[List[str]] = None,
    ...
    def _render_with_textual_datatable(
        ...
        for key in sorted_keys:
            ...
            overflow, no_wrap = self._column_overflow_strategy(key, col_width)
    ...
    def _column_overflow_strategy(
        self, key: str, col_width: int
    ) -> tuple[Literal["fold", "crop", "ellipsis", "ignore"], bool]:
        """Return Rich overflow and wrap settings for a table column."""
        normalized_key = key.lower()
        long_text_fields = {"description", "summary", "details", "message"}
        id_fields = {"id", "client_id", "clientid", "deployment_id", "product_id"}

        if normalized_key in long_text_fields:
            return ("ellipsis", True) if col_width < 20 else ("fold", False)

        if normalized_key in id_fields:
            return "ellipsis", True

        if "_at" in normalized_key or "date" in normalized_key or "time" in normalized_key:
            return "ellipsis", True

        return ("ellipsis", True) if col_width < 15 else ("fold", False)
