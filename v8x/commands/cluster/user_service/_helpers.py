...
"""Shared helpers for user service CLI commands."""
...
def get_username(ctx: typer.Context) -> str:
    """Return the current Vantage username from the CLI persona."""
    persona = getattr(ctx.obj, "persona", None)
    identity_data = getattr(persona, "identity_data", None)
    username = getattr(identity_data, "username", None)
    if not username:
        raise Abort("Missing username in persona.", subject="Authentication Required")
    return username
