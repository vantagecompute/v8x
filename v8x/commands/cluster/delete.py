...
"""Delete cluster command for v8x."""
...
async def _handle_orphaned_deployments(
    ...
    if app:
        ...
        if not json_output:
            ctx.obj.console.print(
                "\n[dim]Use 'v8x app deployment cleanup-orphans' to clean up all orphaned deployments.[/dim]"
            )
        ...
        raise Abort(
            ...
            hint="Use 'v8x app deployment cleanup-orphans' to clean them up.",
        )
...
async def delete_cluster(
    ...
    cluster_type: Annotated[
        ...
        typer.Option(
            "--cluster-type",
            "-c",
            help="Cluster type for deployment cleanup (e.g., k8s, slurm). Required if --app is specified.",
    ...
    app: Annotated[
        ...
        typer.Option(
            "--app",
            help="Cleanup the specified app deployment (e.g., slurm-lxd-localhost, slurm-multipass, slurm-microk8s-localhost, juju-ext)",
            case_sensitive=False,
        ),
    ] = None,
    ...
    # Validate: if --app is specified, --cluster-type is required
    if app and not cluster_type:
        raise Abort(
            "--cluster-type is required when --app is specified for local deployment cleanup.",
            subject="Missing Option",
            log_message="--cluster-type required with --app",
        )
...
async def _call_app_remove_function(ctx: typer.Context, deployment: Deployment) -> None:
    ...
    # Look up app scoped to the deployment's cloud type
    cloud_id = deployment.cloud.id if deployment.cloud else None
    app = deployment_app_sdk.get(deployment.app_name, cloud=cloud_id)
    if app is None:
        # Fallback: try without cloud scope
        app = deployment_app_sdk.get(deployment.app_name)
    if app is None:
