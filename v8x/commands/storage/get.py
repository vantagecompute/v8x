...
async def get_storage(
    ctx: typer.Context,
    name: Annotated[str, typer.Argument(help="Name of the PVC to retrieve")],
    ...
    try:
        ...
        renderer = RenderStepOutput(
            ...
            operation_name=f"Get PVC '{name}'",
            step_names=[] if json_output else ["Fetching PVC", "Done"],
            verbose=verbose,
            command_start_time=command_start_time,
        )

        async with get_http_client(ctx) as client:
            response = await client.get(f"{vdeployer_url}/storage/pvc/{namespace}/{name}")
        ...
        if response.status_code == 200:
            ...
            with renderer:
                renderer.complete_step("Fetching PVC")
                formatter.render_get(data=result, resource_name="PVC")
            ...
            raise Abort(
                f"Failed to get PVC: {response.text}",
                subject="Get PVC Failed",
            )
