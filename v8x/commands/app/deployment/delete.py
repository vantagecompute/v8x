...
async def _find_deployment(ctx, deployment_id):
    """Find a deployment by ID or name.

    Args:
        ctx: Typer context
        deployment_id: Deployment ID or name to search for
...
async def delete_deployment(
    ...
    try:
        deployment = await _find_deployment(ctx, deployment_id)
        ...
        if deployment is None:
            raise Abort(
                ...
                hint="Use 'v8x app deployment list' to see available deployments.",
            )
        ...
        app = deployment_app_sdk.get(app_name, cloud=cloud_id)
        if app is None:
