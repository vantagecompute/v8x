...
def _launch_vm_instance(
    ...
    try:
        ...
        cpu_count = cpu or str(os.cpu_count() or 2)
        memory_size = memory or "4GB"
        disk_size = disk or "10GB"
...
async def create(ctx: typer.Context, cluster: Cluster) -> typer.Exit:
    ...
    multipass_options = getattr(ctx.obj, "slurm_multipass_options", {}) or {}
    ...
    try:
        _launch_vm_instance(
            ...
            cloud_init_config,
            image_origin,
            cpu=multipass_options.get("cpu"),
            memory=multipass_options.get("mem"),
            disk=multipass_options.get("disk"),
        )
