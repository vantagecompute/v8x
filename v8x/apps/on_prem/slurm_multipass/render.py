...
def show_deployment_error(console: Console, cluster_name: str, error: Exception) -> None:
    ...
    console.print(
        f"• Retry with verbose output: [cyan]v8x app deployment {APP_NAME} create {cluster_name} --verbose[/cyan]"
    )
    console.print(
        f"• Remove failed deployment: [cyan]v8x app deployment {APP_NAME} remove <deployment-name>[/cyan]"
    )
    console.print()
...
def success_create_message(deployment: Deployment) -> str:
    ...
    return dedent(
        ...
        [bold]Cleanup:[/bold]
        • Remove deployment: [cyan]v8x app deployment {APP_NAME} remove {instance_name}[/cyan]
...
def success_destroy_message(deployment: Deployment) -> str:
    return dedent(
        ...
        • Verify removal: [cyan]v8x app deployments[/cyan]
        • Deploy new cluster: [cyan]v8x cluster create mynewccluster --app slurm-juju-localhost --cloud localhost[/cyan]
        """
    )
