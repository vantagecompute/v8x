...
"""Configuration settings for v8x."""
...
class Settings(BaseModel):
    """Configuration settings for the v8x."""
...
def clear_settings() -> None:
    """Remove the entire v8x local user directory."""
    logger.debug(f"Removing entire v8x directory at {V8X_LOCAL_USER_BASE_DIR}")
    try:
        shutil.rmtree(V8X_LOCAL_USER_BASE_DIR)
        logger.debug(f"Removed v8x directory at {V8X_LOCAL_USER_BASE_DIR}")
    except FileNotFoundError:
        logger.debug("v8x directory already absent; nothing to remove")


def ensure_default_profile_exists() -> None:
    """Ensure the default profile exists and is properly configured."""
    if V8X_LOCAL_USER_BASE_DIR.exists() is False:
        V8X_LOCAL_USER_BASE_DIR.mkdir(parents=True, exist_ok=True)
        logger.debug(f"Created local user base directory at {V8X_LOCAL_USER_BASE_DIR}")
...
def set_active_profile(profile_name: str) -> None:
    ...
    V8X_ACTIVE_PROFILE.parent.mkdir(parents=True, exist_ok=True)
    V8X_ACTIVE_PROFILE.write_text(profile_name)
